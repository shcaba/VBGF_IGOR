rv$gompertz_summaries
# Gompertz

model = nls(len ~ a * exp(-b * exp(-k * age)),
    data = list(age = age_use, len = len_use),
    start = list(a = input$gom_a_nls, b = input$gom_b_nls, k = input$gom_k_nls),
    control = list(reltol=0.00000000001))

rep = summary(model)[["parameters"]]
newRow = format(data.frame("Run name" = input$runname_nls,
                           "Starting a" = input$gom_a_nls,
                           "Starting b" = input$gom_b_nls,
                           "Starting k" = input$gom_k_nls,
                           "a mean" = rep[1,1],
                           "a Std" = rep[1,2],
                           "b mean" = rep[2,1],
                           "b Std" = rep[2,2],
...
parameters = list(a = input$gom_a_nls, b = input$gom_b_nls, k = input$gom_k_nls, CV_Lt = CV_Lt)
